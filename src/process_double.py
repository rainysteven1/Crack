...
from core.losses import BCEDiceLoss
from core.metrics import IOU, MetricTracker
from core.unet import DoubleUNet
...
class Process:
    def __init__(
        ...
        self.model = DoubleUNet(self.input_dim, self.output_dim).to(DEVICE)
    ...
    def train(
        ...
        for epoch in range(1, epochs + 1):
            ...
            for x, y in train_loader:
                ...
                y_pred_1, y_pred_2 = self.model(x)
                y_pred_1 = y_pred_1.view(-1)
                y_pred_2 = y_pred_2.view(-1)
                loss = self.criterion(y_pred_1, y) + self.criterion(y_pred_2, y)
                loss.backward()
                self.optimizer.step()
                train_metric_record.update(
                    loss.item(),
                    IOU(y_pred_2, y).item(),
                    self.metric(y_pred_2, y).item(),
                )
    ...
    def predict(
        ...
        y_pred_1_all = torch.empty_like(
            y_true_all,
            device=self.device,
            dtype=torch.float32,
        )
        y_pred_2_all = torch.empty_like(
            y_true_all,
            device=self.device,
            dtype=torch.float32,
        )
        ...
        with torch.no_grad():
            for x, y in loader:
                ...
                y_pred_1_all = torch.cat((y_pred_1_all, y_pred_1), dim=0)
                y_pred_2_all = torch.cat((y_pred_2_all, y_pred_2), dim=0)

        def log_process(function):
            self.logger.info("Metric(Pred_1):")
            function(y_true_all, y_pred_1_all)
            self.logger.info()
            self.logger.info("Metric(Pred_2):")
            function(y_true_all, y_pred_2_all)
        ...
        y_pred_1_all = y_pred_1_all.detach().cpu().numpy()
        y_pred_2_all = y_pred_2_all.detach().cpu().numpy()
        ...
        return {
            ...
            "Prediction": {
                "Prediction_1": {"data": y_pred_1_all, "is_gray": True},
                "Prediction_2": {"data": y_pred_2_all, "is_gray": True},
            },
        }
