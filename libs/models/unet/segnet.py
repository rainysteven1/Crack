...
class _EncoderBlock(nn.Sequential):
    def __init__(self, input_dim: int, output_dim: int, n_block: int) -> None:
        super().__init__(
            *(
                [SingleBlock(input_dim, output_dim)]
                + [SingleBlock(output_dim, output_dim) for _ in range(n_block - 1)]
            )
        )
...
class _DecoderBlock(nn.Sequential):
    def __init__(self, input_dim: int, output_dim: int, n_block: int) -> None:
        super().__init__(
            *(
                [SingleBlock(input_dim, input_dim) for _ in range(n_block - 1)]
                + [SingleBlock(input_dim, output_dim)]
            )
        )
...
class SegNet(nn.Module):
    def __init__(
        ...
        self.output_layer = nn.Sequential(
            SingleBlock(decoder_filters[-1], decoder_filters[-1]),
